...
{
 extern class cInfo;
 extern std::vector<cInfo> countries;
...
//P/         sentmsg.set("Sent %s to leader of %s" % (gifts[gift.get()], name))//P/ def sendGift(*args):
//P/     idxs = lbox.curselection()
//P/     if len(idxs)==1:
//P/         idx = int(idxs[0])
//P/         lbox.see(idx)
...
//P/         statusmsg.set("The population of %s (%s) is %d" % (name, code, popn))
 censusmsg= census + country.name + " (" + country.code +") is " + to_string(country.population);
//P/     sentmsg.set('') 
...
//P/ g1 = ttk.Radiobutton(c, text=gifts['card'], variable=gift, value='card')//
 Tk::radiobutton(".f.rb1") -text("Greeting card") -variable(gift) -value("card");
//P/ g2 = ttk.Radiobutton(c, text=gifts['flowers'], variable=gift, value='flowers')
 Tk::radiobutton(".f.rb2") -text("Flowers") -variable(gift) -value("flowers");
//P/ g3 = ttk.Radiobutton(c, text=gifts['nastygram'], variable=gift, value='nastygram')
 Tk::radiobutton(".f.rb3") -text("nastrygram") -variable(gift) -value("nastygram");
//P/ send = ttk.Button(c, text='Send Gift', command=sendGift, default='active')
 Tk::button(".f.b1") -text("Send Gift "+gift);
...
//P/ status = ttk.Label(c, textvariable=statusmsg, anchor=W)
 ...
 Tk::label(".f.l4") -textvariable(censusmsg);
